...
# Gemini free tier allows 15 requests/min, and contextual precision spends 2 requests per test case (verdicts, reason).
...
with open("goldens/retriever_goldens.json", "r") as f:
    contextual_precision_goldens = json.load(f)

test_cases = []

for golden in contextual_precision_goldens:
    query = golden["query"]
